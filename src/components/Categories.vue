<template>
  <div id="categories" class="container-sm">
    ...
    <div class="btn-group btn-group-sm pt-2" role="group">
      <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addCategoryForm"
              @click="isEdit=false">New</button>
      <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addCategoryForm"
              :disabled="selectedCategory.id === -1" @click="isEdit=true">Edit</button>
    </div>
    ...
    <AddCategoryForm @updateCategories="$emit('updateCategories'); selectedCategory.id = -1" :categories="categories"
                     :isEdit="isEdit" :id="selectedCategory.id"/>
  </div>
</template>
